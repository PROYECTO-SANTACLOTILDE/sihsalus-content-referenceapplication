...
- Configura ubicaciones con los tags `Queue Location` y `Appointment Location` provistos por sus módulos, y define tipos de servicio de cita para que agenda no quede sin duración.
...
### Cambiado
- Evita la colisión de Initializer con los tags `Queue Location` y `Appointment Location`: el content package deja de recrearlos y valida que permanezcan a cargo de los módulos de colas y citas.
